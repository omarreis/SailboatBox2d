unit uDebugDrawerFM;  //TDebugDrawerFM

interface
{$I ..\..\Physics2D\Physics2D.inc}

uses
  System.UITypes, UPhysics2DTypes, UPhysics2D, uDebugDrawer,
  System.Types, FMX.Graphics, System.Math.Vectors;
...
type
  ...
  public
    ...
    function    ConvertWorldToScreen(const x,y: Single): uPhysics2DTypes.TVector2;  //Om:
    procedure   CenterOnWorldCoordinates(const x, y: Single);                       //Om:
    ...
    procedure DrawSmallText(const pt: TVector2; const Text:String; const color: RGBA); override;  //Om:
...
function TDebugDrawerFM.ConvertScreenToWorld(x,y: Single): uPhysics2DTypes.TVector2;
begin
  Result.x := (x - offsetX) / scaleX;
  Result.y := ((CanvasHeight-y) - offsetY) / scaleY;
end;

function  TDebugDrawerFM.ConvertWorldToScreen(const x,y: Single): uPhysics2DTypes.TVector2;  //Om:
begin
  Result.x :=   x * scaleX + offsetX;
  Result.y := -(y * scaleY + offsetY-CanvasHeight);
end;

Procedure TDebugDrawerFM.CenterOnWorldCoordinates(const x,y: Single);  //Om: center in w, keeping scale
var c:TVector2;
begin
  c  := MakeVector(200,300);  //Om: since here we don't know the view port dimensions (canvas size), use (200,300)
  offsetX := +c.x - x*scaleX;
  offsetY := -c.y - y*scaleY+CanvasHeight;
end;
...
procedure TDebugDrawerFM.DrawPolygon(const vertices: Tb2PolyVertices;  vertexCount: Int32; const color: RGBA);
...
procedure TDebugDrawerFM.DrawSmallText(const pt: TVector2; const Text:String; const color: RGBA);  //Om:
var ARect: TRectF;  aColor: TColor; aP1: TPointF;
begin
  aColor := RGBAToFMColor(color);
  aP1.X := offsetX + pt.x * scaleX;                    //convert to screen
  aP1.Y := CanvasHeight - (offsetY + pt.y * scaleY);
  // TODO: sanity test in aP1 coordinates ??
  ARect := RectF( aP1.x, aP1.y, aP1.x+Canvas.TextWidth(text), aP1.y+Canvas.TextHeight(text) );
  Canvas.Fill.Color := aColor;
  Canvas.Font.Size := 10; //sml

  Canvas.FillText(ARect, Text, false, DEFAULT_OPACITY, [], TTextAlign.taLeading, TTextAlign.taLeading);
end;
