unit uFormDBox2DTestBedM;         // --- form testbed box2d for firemonkey ---\\
   //----------------------------//                                            \\--------------
  // Om: added a dirt keyboard
...
uses
  ...
  FMX.Layouts, FMX.Edit, FMX.TreeView, FMX.ListBox, FMX.Objects, FMX.Gestures,
  FMX.TabControl, FMX.Controls.Presentation, System.Math,
  uXCadencer, uDebugDrawerFM, uTestBed,  uPhysics2D, uPhysics2DTypes;  //b2d
...
type
  TFormDBox2DTestBedM = class(TForm, ITesterForm)
    ...
    pnlTeclas: TPanel;
    btnKeyA: TSpeedButton;
    btnKeyD: TSpeedButton;
    btnKeyW: TSpeedButton;
    btnKeyS: TSpeedButton;
    btnKeyJ: TSpeedButton;
    btnKeyK: TSpeedButton;
    btnKeyM: TSpeedButton;
    btnKeyN: TSpeedButton;
    btnKeyU: TSpeedButton;
    btnKeyI: TSpeedButton;
    btnKeySpace: TSpeedButton;
    btnKeyPlus: TSpeedButton;
    btnKeyMinus: TSpeedButton;
    btnKeyZ: TSpeedButton;
    btnKeyX: TSpeedButton;
    btnKeyC: TSpeedButton;
    btnKeyV: TSpeedButton;
    btnKeyQ: TSpeedButton;
    btnKeyF: TSpeedButton;
    btnKeyG: TSpeedButton;
    btnKeyPt: TSpeedButton;
    btnKeyComa: TSpeedButton;
    btnKeySlash: TSpeedButton;
    ...
    procedure pntbxDrawPanelMouseDown(Sender: TObject; Button: TMouseButton;   Shift: TShiftState; X, Y: Single);
    procedure pntbxDrawPanelMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Single);
    procedure pntbxDrawPanelMouseUp(Sender: TObject; Button: TMouseButton;   Shift: TShiftState; X, Y: Single);
    procedure BackBTNClick(Sender: TObject);
    procedure pntbxDrawPanelGesture(Sender: TObject; const EventInfo: TGestureEventInfo; var Handled: Boolean);
    procedure btnLaunchBombClick(Sender: TObject);
    procedure btnTeclaClick(Sender: TObject);
  private
...
procedure TFormDBox2DTestBedM.FormKeyDown(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
var s: string;
begin
  if Key = vkEscape then             Close
  else if Key = vkLeft then
   if ssCtrl in Shift then Test.ShiftOrigin(MakeVector(2.0, 0))
    else begin
   ...
   if ssCtrl in Shift then Test.ShiftOrigin(MakeVector(0.0, -2.0))
    else begin
  ...
  else if Key = vkHome then ResetView
  else if (KeyChar = 'P') or (KeyChar = 'p') then btnPauseClick(nil)
  else if (KeyChar = '+') then      //Om: + - used for Zoom here on iPhone may be used in the test also  :(
    begin
      FDrawer.ScaleX := FDrawer.ScaleX*1.1;
      FDrawer.ScaleY := FDrawer.ScaleY*1.1;
    end
  ...
  begin
    if KeyChar = ' ' then
      Test.LaunchBomb
    else Test.Keyboard(Ord(KeyChar.ToUpper));
  end;
end;
...
begin
  ...
  FDrawer.ScaleX := 10;
  FDrawer.ScaleY := 10;
  FDrawer.CanvasHeight := pntbxDrawPanel.Height+(pntbxDrawPanel.Height/4);  //canvas a little larger than actual
end;
...
begin
  ...
  try
    ...
    mRect := RectF(0,0,pntbxDrawPanel.Width,pntbxDrawPanel.Height); //pntbxDrawPanel.BoundsRect;
...
procedure TFormDBox2DTestBedM.btnTeclaClick(Sender: TObject);
var c:Char; S:String; Key: Word;
begin
  S:=(Sender as TSpeedButton).Text;
  if S<>'' then
    begin
      c:=S[Low(S)];
      Key:=0;
      FormKeyDown(nil,Key,{KeyChar=}C, {Shift=}[] );    //simula tecla
    end;
end;
