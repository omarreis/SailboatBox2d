...
begin
   ...
   if Assigned(Callback) then
      windforce := Callback()      // get wind vector
   else
   ...
   while Assigned(i) do
      ...
      begin
         ...
         shape := GetFixtureList;  //get 1st fixture
         while Assigned(shape) do
         begin
            if shape.GetType = e_circleShape then
               ...
               {$IFDEF OP_OVERLOAD}
               appliedforce.AddBy( ComputeEdgeShapeEffectiveForce(windforce, Tb2EdgeShape(shape.GetShape), GetTransform^) );
               {$ELSE}
               AddBy(appliedforce, ComputeEdgeShapeEffectiveForce(windforce,Tb2EdgeShape(shape.GetShape), GetTransform^));
               {$ENDIF}
            shape := shape.GetNext;
         end;
...
begin
   ...
   with shape do
   begin
     _ComputeSegmentEffectiveForce(m_vertex1, m_vertex2, m_normal1, xf, windforce, Result);
