...
// Om: Out16: 2 bugs corrected for 64b compilers
...
begin
   ...
   m_jointLineColor :=MakeColor(0.2, 0.4, 0.4);    //Om: was MakeColor(0.5, 0.8, 0.8);
end;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            while Assigned(f) do
            begin
               if not b.IsActive                   then DrawShape(f, b.m_xf, m_shapeColor_Inactive)
               else if b.m_type = b2_staticBody    then DrawShape(f, b.m_xf, m_shapeColor_Static)
               else if b.m_type = b2_kinematicBody then DrawShape(f, b.m_xf, m_shapeColor_kinematic)
               else if not b.IsAwake               then DrawShape(f, b.m_xf, m_shapeColor_Sleeping)
               else                                     DrawShape(f, b.m_xf, m_shapeColor_Normal);
               f := f.m_next;
            end;
            b := b.m_next;
         end;
...
begin
   ...
   //was PInt32( Integer(m_stack) + SizeOf(Int32) * m_count)^ := element;  //Om: this uses 32b pointer arithmetic. Fails in 64 bit systems
   PInt32( NativeUInt(m_stack) + SizeOf(Int32) * m_count)^ := element; //Om: corrected out16
...
begin
   //b2Assert(m_count > 0);
	 Dec(m_count);
	 //was Result := PInt32(Integer(m_stack) + SizeOf(Int32) * m_count)^;
         Result := PInt32(NativeUInt(m_stack) + SizeOf(Int32) * m_count)^; //Om: corrected out16
end;
